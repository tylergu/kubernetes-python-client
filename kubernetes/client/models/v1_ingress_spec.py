...
class V1IngressSpec(BaseModel):
    ...
    rules: Optional[List[V1IngressRule]] = Field(default=None, description="A list of host rules used to configure the Ingress. If unspecified, or no rule matches, all traffic is sent to the default backend.")
    tls: Optional[List[V1IngressTLS]] = Field(default=None, description="TLS configuration. Currently the Ingress only supports a single TLS port, 443. If multiple members of this list specify different hosts, they will be multiplexed on the same port according to the hostname specified through the SNI TLS extension, if the ingress controller fulfilling the ingress supports SNI.")
